...
**현재 버전: `0.2.0`** — 변경 이력은 [CHANGELOG.md](CHANGELOG.md) 참조.

macOS 메뉴바 native 앱. `~/.claude/` 의 플러그인·마켓·스킬·훅·MCP 서버를 한 화면에서 관리.

![Claude Code Plugin Manager — 메인 윈도우](docs/screenshots/main-window.png)

> 상세 사양은 [PRD.md](PRD.md) 참조 (v2.2, ~2,000줄).
> 릴리스 이력 및 버전 정책: [CHANGELOG.md](CHANGELOG.md).
...
- **MCP 관리** *(0.2.0 신규)* — MCPs 탭에서 설치된 MCP 서버를 disable / enable / remove (user-scope + plugin-bundled 모두).
- **버전 가시성** — 플러그인 버전 컬럼 + 마켓별 `metadata.version` 캡슐 + 마지막 refresh 시각.
- **자동 새로고침** — `~/.claude/` 트리 (그리고 `~/.claude.json`) 변경을 FSEvents 로 감지해 debounced reload.
...
├── Scripts/                 # 배포 파이프라인
│   ├── install-local.sh     # /Applications 로컬 설치 + login item
...
- **MCPs**: user-scope (`~/.claude.json#mcpServers`) + plugin-bundled (`<plugin>/.mcp.json`) MCP 통합 목록. Toggle 은 모든 현재 프로젝트의 disable 배열에 일괄 적용. 휴지통 버튼은 user-scope 등록만 제거 (`claude mcp remove` 위임). plugin-bundled MCP 는 disable 만 가능 — 완전 제거하려면 호스트 플러그인을 uninstall.
- **Diagnostics**: 디스크 무결성 체크 + Orphaned Cache cleanup (Q2 spike leftover 처리).

## 마일스톤 진행 상황

- ✅ M0 (spike + 인프라) — Q2/Q8 spike 완료
...
- ✅ **0.2.0** — MCPs 탭 (disable / enable / remove), `~/.claude.json` watching, 109개 unit test
